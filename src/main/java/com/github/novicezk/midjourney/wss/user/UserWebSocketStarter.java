...
public class UserWebSocketStarter extends WebSocketAdapter implements WebSocketStarter {
    ...
    public UserWebSocketStarter(ProxyProperties properties) {
        ...
        String baseUrl = this.properties.getNg().getWss();
        if (!baseUrl.endsWith("/")) {
            baseUrl += "/";
        }
        this.gatewayUrl = baseUrl + "?encoding=json&v=9&compress=zlib-stream";
    ...
    public synchronized void start() throws Exception {
        ...
        this.socket = webSocketFactory.createSocket(gatewayUrl);
    ...
    private DataObject createAuthData() {
        ...
        DataObject clientState =
            DataObject.empty().put("guild_hashes", DataArray.empty()).put("highest_last_message_id", "0")
